...
function exibirVideosSalvos() {
    ...
    db.collection("videos").get()
        .then(function (querySnapshot) {
            querySnapshot.forEach(function (doc) {
                // Criar um item da lista com o vídeo e botão de exclusão
                var itemLista = document.createElement("li");
                var tagVideo = doc.data().tagVideo;
                var videoElement = document.createElement("iframe");
                videoElement.classList.add("video"); // Agregar la clase 'video'
                videoElement.src = "https://www.youtube.com/embed/" + tagVideo;
                itemLista.appendChild(videoElement);

                var deleteButton = document.createElement("button");
                deleteButton.dataset.id = doc.id;
                deleteButton.classList.add("btnExcluir");
                deleteButton.textContent = "Excluir";
                itemLista.appendChild(deleteButton);

                listaVideos.appendChild(itemLista);

                // Resto del código de evento para el botón de exclusión
            });
        })
        .catch(function (error) {
            console.error("Erro ao buscar vídeos: ", error);
